...
const thorLabsScraper = async (url) => {
  ...
  let nameswithDuplicates = namesUntidy
    .filter((item) => item.startsWith("LA"))
    .filter((item) => item.length < 7);

  let names = [...new Set(nameswithDuplicates)];

  const prices = await page.$$eval(
    'td[class="CSS4"][align="right"][style="padding-right: 15px;"]',
    (prix) => {
      return prix.map((x) => x.textContent.replace("\n", "").trim());
    }
  );

  let allFeatures = await page.$$eval(
  ...
  let allFeaturesWithoutEmptyStrings = allFeatures.filter(
    (item) => item.length > 0
  );
  ...
  let multidimensionalAllFeatures = allFeaturesWithoutEmptyStrings.reduce(
    (a, c, i) => {
      return i % 7 === 0
        ? a.concat([allFeaturesWithoutEmptyStrings.slice(i, i + 7)])
        : a;
    },
    []
  );
